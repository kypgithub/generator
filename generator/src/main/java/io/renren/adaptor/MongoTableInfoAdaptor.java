...
public class MongoTableInfoAdaptor {
     ...
     * 此方法只针对主Bean
     */
    public static List<Map<String, Object>> columnInfo(MongoDefinition mongoDefinition) {
        List<MongoDefinition> child = mongoDefinition.getChild();
        List<Map<String, Object>> result = new ArrayList<>(child.size());
        final String mongoKey = "_id";
        for (MongoDefinition definition : child) {
            Map<String, Object> map = new HashMap<>(5 * 4 / 3 + 1);
